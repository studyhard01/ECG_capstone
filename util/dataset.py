...
LABEL = {
    'ptb_super': ['HYP', 'NORM', 'MI', 'CD', 'STTC'],
    'ptb_super_nonomal': ['HYP', 'MI', 'CD', 'STTC'],
    'ptb_sub': ['AMI', 'LAFB/LPFB', 'LVH', 'STTC', 'IMI', 'SEHYP', 'CRBBB', 'WPW', 'LAO/LAE', 'NORM', 'ISC', 'AVB', 'RAO/RAE', 'LMI', 'ISCI', 'ISCA', 'NST', 'CLBBB', 'ILBBB', 'IRBBB', 'PMI', 'RVH', 'IVCD'],
    'ptb_rhythm': ['SVARR', 'BIGU', 'STACH', 'SARRH', 'SBRAD', 'TRIGU', 'AFIB', 'SR', 'SVTAC', 'PSVT', 'AFLT', 'PACE'],
    'ptb_form': ['PAC', 'DIG', 'HVOLT', 'STD', 'LPR', 'QWAVE', 'VCLVH', 'NDT', 'TAB', 'LOWT', 'ABQRS', 'LNGQT', 'PRC(S)', 'NT', 'STE', 'NST', 'PVC', 'INVT', 'LVOLT']
}
...
def build_dataset(cfg: dict, split: str) -> ECGDataset:
    ...
    """
    fname_col = cfg.get("filename_col", "FILE_NAME") # ecg 신호 존재 파일 경로 열
    fs_col = cfg.get("fs_col", None) # smple rate 열
    label_col = cfg.get("label_col", None) # label 열 (나는 여기를 고쳐야 함 !! multi_class로 간다면 !)
    target_lead = cfg.get("lead", "12lead") # lsed 어떻게?
    target_fs = cfg.get("fs", 250) # 주파수 어떻게?

    label_type = cfg.get("label_type", None) ## 내가 추가 (0925)
    ...
    # labels = df[label_col].astype(int).values if label_col is not None else None ## 수정 필요!!
    labels = np.array(df[LABEL[label_type]]) if label_type is not None else None
