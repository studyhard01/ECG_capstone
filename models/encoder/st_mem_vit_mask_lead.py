...
class ST_MEM_ViT(nn.Module):
    ...
    def forward_encoding(self, series):
        ...
        # lead_embeddings = torch.stack([lead_embedding for lead_embedding in self.lead_embeddings]).unsqueeze(0) # [1, 12, 768]
        # lead_embeddings = lead_embeddings.unsqueeze(2).expand(b, -1, n + 2, -1)
        lead_embeddings = self.lead_embeddings[-1].unsqueeze(0).unsqueeze(0).expand(b, -1, n + 2, -1)
        x = x + lead_embeddings
...
'''
class ST_MEM_ViT_shffle(nn.Module):
    def __init__(self,
                 seq_len: int,
                 patch_size: int,
                 num_leads: int,
...
'''
class ST_MEM_ViT_random(nn.Module):
    def __init__(self,
                 seq_len: int,
                 patch_size: int,
                 num_leads: int,
